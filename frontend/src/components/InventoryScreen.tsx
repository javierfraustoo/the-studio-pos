          ...
          return (
            <details key={item.id} style={styles.row}>
              <summary style={{ ...styles.rowSummary, backgroundColor: isLow ? 'var(--danger-bg)' : 'var(--bg-card)' }}>
                ...
                <span style={{ ...styles.td, color: expiryDays !== null && expiryDays <= 3 ? '#DC2626' : '#6B7280' }}>
              ...
              <div style={styles.batchDetail}>
                ...
                {activeBatches.map((b, i) => (
                  <div key={b.id} style={{ ...styles.batchRow, backgroundColor: i === 0 ? '#FFFBEB' : '#FFF' }}>
...
const styles: Record<string, React.CSSProperties> = {
  ...
  batchTh: { flex: 1, fontSize: 10, fontWeight: 700, color: '#9CA3AF', textTransform: 'uppercase' as const },
  batchRow: { display: 'flex', padding: '6px 0', borderBottom: '1px solid #F9FAFB' },
  batchTd: { flex: 1, fontSize: 12, color: '#374151' },
  fifoTag: { fontSize: 10, fontWeight: 700, color: '#D97706', backgroundColor: '#FEF3C7', padding: '1px 5px', borderRadius: 3, marginRight: 4 },
};
